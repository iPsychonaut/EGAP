...
def validate_fasta(file_path):
    if not file_path:
        log_print("ERROR:\tFASTA path is empty/None")
        return False
...
def long_reads_purge_dups(curation_out_dir, polished_assembly, ont_raw_reads,
    ...
    if pd.notna(ont_raw_reads) and pd.isna(pacbio_raw_reads):
        ...
        log_print("WARN:\tNo valid long reads (ONT or PacBio) provided for purge_dups. Skipping.")
        os.chdir(cwd)
    ...
    if not validate_fasta(polished_assembly):
        log_print(f"ERROR:\tInvalid polished assembly: {polished_assembly}")
        os.chdir(cwd)
        return polished_assembly
    if not os.path.exists(highest_mean_qual_long_reads):
        log_print(f"ERROR:\tLong reads not found: {highest_mean_qual_long_reads}")
        os.chdir(cwd)
        return polished_assembly

    log_print(f"DEBUG - Using polished assembly: {polished_assembly}")
    log_print(f"DEBUG - Using long reads: {highest_mean_qual_long_reads}")
    log_print(f"DEBUG - Writing FOFN to: {pd_fofn}")

    if os.path.exists(pd_json):
        log_print(f"SKIP:\tPurge Dupes JSON already exists: {pd_json}. Overwriting to ensure fresh run.")
        os.remove(pd_json)

    pd_config_path = shutil.which("pd_config.py")
    if not pd_config_path:
        log_print("WARN:\tpd_config.py not found on PATH. Skipping purge_dups.")
        os.chdir(cwd)
        return dup_purged_assembly

    purge_dupes_config_cmd = ["python3", pd_config_path, polished_assembly,
                              pd_fofn, "-l", curation_out_dir, "-n", pd_json]
    log_print(f"DEBUG - Running purge_dups config: {' '.join(purge_dupes_config_cmd)}")
    result = run_subprocess_cmd(purge_dupes_config_cmd, shell_check=False)
    if result != 0:
        log_print(f"WARN:\tFailed to generate purge_dups config: pd_config.py returned {result}")
        os.chdir(cwd)
        return dup_purged_assembly

    if os.path.exists(dup_purged_assembly):
        log_print(f"SKIP:\tDuplicate Purged Assembly already exists: {dup_purged_assembly}. Overwriting to ensure fresh run.")
        os.remove(dup_purged_assembly)

    run_purge_dups_path = shutil.which("run_purge_dups.py")
    if not run_purge_dups_path:
        log_print("WARN:\trun_purge_dups.py not found on PATH. Skipping purge_dups.")
        os.chdir(cwd)
        return dup_purged_assembly

    purge_dupes_cmd = ["python3", run_purge_dups_path, pd_json, os.path.dirname(run_purge_dups_path),
                       sample_id, "-p", "bash"]
    log_print(f"DEBUG - Running purge_dups: {' '.join(purge_dupes_cmd)}")
    result = run_subprocess_cmd(purge_dupes_cmd, shell_check=False)
    if result != 0:
        log_print(f"WARN:\tFailed to run purge_dups: run_purge_dups.py returned {result}")

    if os.path.exists(purged_output):
        log_print(f"DEBUG - Copying purge_dups output: {purged_output} to {dup_purged_assembly}")
        shutil.copy(purged_output, dup_purged_assembly)
    else:
        log_print(f"WARN:\tPurge_dups output not found: {purged_output}")
        os.chdir(cwd)
        return dup_purged_assembly

    os.chdir(cwd)
    log_print(f"PASS:\tCompleted purge_dups curation: {dup_purged_assembly}.")
    return dup_purged_assembly
...
def ref_seq_ragtag(dup_purged_assembly, ref_seq, curation_out_dir, sample_id, cpu_threads, ram_gb):
    ...
    if not validate_fasta(dup_purged_assembly):
        log_print(f"ERROR:\tInvalid duplicate-purged assembly: {dup_purged_assembly}")
        return final_assembly
    if not (ref_seq and os.path.exists(ref_seq)):
        log_print(f"ERROR:\tReference sequence not found: {ref_seq}")
        return final_assembly
    ...
    if os.path.exists(ragtag_ref_assembly):
        ...
        if os.path.exists(scaff_fasta):
            ...
            log_print(f"DEBUG - Running RagTag scaffold: {' '.join(ragscaf_cmd)}")
            result = run_subprocess_cmd(ragscaf_cmd, shell_check=False)
            if result != 0 or not os.path.exists(scaff_fasta):
                log_print(f"WARN:\tRagTag scaffold failed: ragtag.py returned {result}")
                return final_assembly
        ...
        if os.path.exists(corr_fasta):
            ...
            ragcorr_cmd = ["ragtag.py", "correct", ref_seq, scaff_fasta,
                           ...
                           "-t", str(cpu_threads), "-u"]
            log_print(f"DEBUG - Running RagTag correct: {' '.join(ragcorr_cmd)}")
            result = run_subprocess_cmd(ragcorr_cmd, shell_check=False)
            if result != 0 or not os.path.exists(corr_fasta):
                log_print(f"WARN:\tRagTag correct failed: ragtag.py returned {result}")
                return final_assembly

        if os.path.exists(patch_fasta):
            log_print(f"SKIP:\tRagTag Patched Assembly already exists: {patch_fasta}.")
        else:
            ragpatch_cmd = ["ragtag.py", "patch", ref_seq, corr_fasta,
                            "-o", ragtag_patched_output_dir,
                            "-t", str(cpu_threads), "-u"]
            log_print(f"DEBUG - Running RagTag patch: {' '.join(ragpatch_cmd)}")
            result = run_subprocess_cmd(ragpatch_cmd, shell_check=False)
            if result != 0 or not os.path.exists(patch_fasta):
                log_print(f"WARN:\tRagTag patch failed: ragtag.py returned {result}")
                return final_assembly

        if os.path.exists(patch_fasta):
            shutil.copy(patch_fasta, ragtag_ref_assembly)
            log_print(f"DEBUG - Copied RagTag output: {patch_fasta} to {ragtag_ref_assembly}")
        else:
            log_print(f"ERROR:\tRagTag patch output not found: {patch_fasta}")
            return final_assembly

    log_print(f"PASS:\tCompleted RagTag reference curation: {ragtag_ref_assembly}.")
...
def long_reads_tgs_gapcloser(curation_out_dir, ragtag_ref_assembly, highest_mean_qual_long_reads, sample_id, cpu_threads):
    ...
    if not validate_fasta(ragtag_ref_assembly):
        log_print(f"ERROR:\tInvalid RagTag-corrected assembly: {ragtag_ref_assembly}")
        return final_assembly
    ...
    if os.path.exists(tgs_gapcloser_output):
        log_print(f"SKIP:\tTGS-GapCloser Assembly already exists: {tgs_gapcloser_output}. Overwriting to ensure fresh run.")
    ...
    tgs_gapcloser_cmd = ["tgsgapcloser",
                         ...
                         "--ne"]
    log_print(f"DEBUG - Running TGS-GapCloser: {' '.join(tgs_gapcloser_cmd)}")
    result = run_subprocess_cmd(tgs_gapcloser_cmd, shell_check=False)
    if result != 0:
        log_print(f"WARN:\tTGS-GapCloser failed: tgsgapcloser returned {result}")
    ...
    if os.path.exists(gap_filled_assembly):
        log_print(f"SKIP:\tGap-Filled Assembly already exists: {gap_filled_assembly}. Overwriting to ensure fresh run.")
        os.remove(gap_filled_assembly)
    if os.path.exists(tgs_gapcloser_output):
        shutil.copy(tgs_gapcloser_output, gap_filled_assembly)
        log_print(f"DEBUG - Copied TGS-GapCloser output: {tgs_gapcloser_output} to {gap_filled_assembly}")
    else:
        log_print(f"WARN:\tTGS-GapCloser output not found: {tgs_gapcloser_output}")
        return final_assembly

    log_print(f"PASS:\tCompleted TGS-GapCloser curation: {gap_filled_assembly}.")
...
def illu_abyss_sealer(curation_dir, ragtag_ref_assembly, illu_f_dedup, illu_r_dedup, cpu_threads):
    ...
    log_print(f"DEBUG - ABySS-Sealer output paths: {sealer_output_file}, {renamed_sealer_output_file}")

    if os.path.isfile(renamed_sealer_output_file):
        log_print(f"SKIP:\tABySS Sealer output file already exists: {renamed_sealer_output_file}.")
    ...
    if os.path.isfile(sealer_output_file):
        ...
        if not os.path.exists(renamed_sealer_output_file):
            log_print(f"DEBUG - Copying {sealer_output_file} to {renamed_sealer_output_file}")
            shutil.copy(sealer_output_file, renamed_sealer_output_file)
    else:
        log_print(f"DEBUG - Checking inputs: ragtag_ref_assembly={ragtag_ref_assembly}, illu_f_dedup={illu_f_dedup}, illu_r_dedup={illu_r_dedup}")
        if not validate_fasta(ragtag_ref_assembly):
            log_print(f"ERROR:\tInvalid RagTag-corrected assembly: {ragtag_ref_assembly}")
            return ragtag_ref_assembly
        if not (illu_f_dedup and illu_r_dedup and os.path.exists(illu_f_dedup) and os.path.exists(illu_r_dedup)):
            log_print(f"ERROR:\tIllumina deduplicated reads not found: {illu_f_dedup}, {illu_r_dedup}")
            return ragtag_ref_assembly
        ...
        log_print(f"DEBUG - Running ABySS-Sealer command: {' '.join(abyss_sealer_cmd)}")
        result = run_subprocess_cmd(abyss_sealer_cmd, shell_check=False)
        if result != 0 or not os.path.exists(sealer_output_file):
            log_print(f"WARN:\tABySS Sealer failed: abyss-sealer returned {result}")
            return ragtag_ref_assembly
        log_print(f"DEBUG - ABySS-Sealer produced output: {sealer_output_file}")
        log_print(f"DEBUG - Copying {sealer_output_file} to {renamed_sealer_output_file}")
        shutil.copy(sealer_output_file, renamed_sealer_output_file)

    log_print(f"PASS:\tCompleted Abyss-Sealer curation: {renamed_sealer_output_file}.")
    return renamed_sealer_output_file
...
def curate_assembly(sample_id, input_csv, output_dir, cpu_threads, ram_gb):
    ...
    if os.path.exists(final_curated_assembly) and os.path.getsize(final_curated_assembly) > 0:
        log_print(f"SKIP:\tFinal curated assembly already present: {final_curated_assembly}")
    ...
    if pd.notna(illumina_f_raw_reads) and pd.notna(illumina_r_raw_reads):
        ...
        log_print(f"DEBUG - Checking Illumina deduplicated reads: {illu_dedup_f_reads}, {illu_dedup_r_reads}")
    ...
    os.makedirs(curation_out_dir, exist_ok=True)
    os.chdir(curation_out_dir)
    log_print(f"DEBUG - CWD (curation_out_dir) - {os.getcwd()}")

    # 1) Purge duplicates (if long reads)
    log_print(f"DEBUG - Starting purge duplicates for {sample_id}")
    if pd.notna(ont_raw_reads) or pd.notna(pacbio_raw_reads):
        log_print("Purging Haplotigs using Long Reads (ONT or PacBio)...")
        ...
        if not os.path.exists(dup_purged_assembly):
            log_print(f"WARN:\tPurge duplicates failed to produce output: {dup_purged_assembly}. Falling back to polished assembly.")
            dup_purged_assembly = polished_assembly
    else:
        log_print("SKIP:\tPurge Duplicates; No Long Reads (ONT or PacBio) Provided.")
        dup_purged_assembly = polished_assembly

    log_print(f"DEBUG - Purge duplicates output: {dup_purged_assembly}")

    # 2) RagTag reference correction (if ref provided)
    log_print(f"DEBUG - Starting RagTag correction for {sample_id}")
    if pd.notna(ref_seq) and os.path.exists(ref_seq):
        log_print("Running RagTag with Reference Sequence...")
        ...
        if not os.path.exists(ragtag_ref_assembly):
            log_print(f"WARN:\tRagTag correction failed to produce output: {ragtag_ref_assembly}. Falling back to purged assembly.")
            ragtag_ref_assembly = dup_purged_assembly
    else:
        log_print(f"SKIP:\tRagTag; No valid Reference Sequence Provided: {ref_seq}")
        ragtag_ref_assembly = dup_purged_assembly

    log_print(f"DEBUG - RagTag output: {ragtag_ref_assembly}")

    # 3) Gap closing
    log_print(f"DEBUG - Starting gap closing for {sample_id}")
    if pd.notna(ont_raw_reads) or pd.notna(pacbio_raw_reads):
        log_print("Running TGS-GapCloser with Long Reads (ONT or PacBio)...")
    ...
    if not validate_fasta(curated_assembly):
        log_print(f"WARN:\tInvalid curated assembly: {curated_assembly}. Falling back to polished assembly.")
        curated_assembly = polished_assembly

    final_curated_assembly = os.path.join(sample_dir, f"{sample_id}_final_curated.fasta")
    if os.path.exists(final_curated_assembly):
        log_print(f"SKIP:\tFinal curated assembly already exists: {final_curated_assembly}. Overwriting to ensure fresh run.")
        os.remove(final_curated_assembly)
    if os.path.exists(curated_assembly):
        shutil.copy(curated_assembly, final_curated_assembly)
        log_print(f"PASS:\tCopied curated assembly to {final_curated_assembly}")
    else:
        log_print(f"WARN:\tCannot copy {curated_assembly} to {final_curated_assembly}: curated assembly does not exist. Copying polished assembly.")
        if os.path.exists(polished_assembly):
            shutil.copy(polished_assembly, final_curated_assembly)
            log_print(f"PASS:\tCopied polished assembly to {final_curated_assembly}")
        else:
            log_print(f"ERROR:\tPolished assembly not found: {polished_assembly}")
            return None

    log_print(f"NOTE:\tKeeping polished assembly for debugging: {polished_assembly}")
    log_print(f"PASS:\tCuration complete for {polished_assembly} -> {final_curated_assembly}")
    os.chdir(sample_dir)
    return final_curated_assembly
