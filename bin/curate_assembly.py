...
def long_reads_purge_dups(curation_out_dir, polished_assembly, ont_raw_reads, illu_raw_f_reads, illu_raw_r_reads, pacbio_raw_reads, highest_mean_qual_long_reads, sample_id, cpu_threads):
    ...
    Args:
        ...
        highest_mean_qual_long_reads (str): Path to gzipped high-quality long reads.
    ...
    if not os.path.exists(highest_mean_qual_long_reads):
        raise FileNotFoundError(f"ERROR:\tLong reads not found: {highest_mean_qual_long_reads}")
    ...
    if os.path.exists(dup_purged_assembly):
        ...
        # Check for purged output even if run_busco2 fails
        if os.path.exists(purged_output):
            print(f"DEBUG - Moving purge_dups output: {purged_output} to {dup_purged_assembly}")
...
# --------------------------------------------------------------
def ont_tgs_gapcloser(curation_out_dir, ragtag_ref_assembly, highest_mean_qual_long_reads, sample_id, cpu_threads):
    ...
    Args:
        ...
        highest_mean_qual_long_reads (str): Path to gzipped high-quality ONT reads.
        sample_id (str): Sample identifier.
        cpu_threads (str): Number of CPU threads to use.
    ...
    if not os.path.exists(ragtag_ref_assembly):
        ...
        return final_assembly
    if not os.path.exists(highest_mean_qual_long_reads):
        raise FileNotFoundError (f"ERROR:\tONT reads not found: {highest_mean_qual_long_reads}")
...
def illu_abyss_sealer(curation_dir, ragtag_ref_assembly, illu_f_dedup, illu_r_dedup, cpu_threads):
    ...
    Args:
        ...
        ragtag_ref_assembly (str): Path to the RagTag-corrected assembly FASTA.
        illu_f_dedup (str): Path to gzipped deduplicated forward Illumina reads.
        illu_r_dedup (str): Path to gzipped deduplicated reverse Illumina reads.
        cpu_threads (str): Number of CPU threads to use.
    ...
    sealer_output_file = f"{output_prefix}_scaffold.fa"  # Corrected to match actual output
    renamed_sealer_output_file = os.path.join(curation_dir, "abyss_sealer_final_curated.fasta")

    print(f"DEBUG - ABySS-Sealer output paths: {sealer_output_file}, {renamed_sealer_output_file}, {renamed_sealer_output_file}")
    ...
    if os.path.isfile(sealer_output_file):
        ...
        if not os.path.exists(renamed_sealer_output_file):
            print(f"DEBUG - Renaming {sealer_output_file} to {renamed_sealer_output_file}")
...
def curate_assembly(sample_id, input_csv, output_dir, cpu_threads, ram_gb):
    ...
    if not os.path.exists(curated_assembly):
        print(f"ERROR:\tCurated assembly not produced: {curated_assembly}")
        return None

    final_curated_assembly = os.path.join(sample_dir, f"{sample_id}_final_curated.fasta")

    if os.path.exists(final_curated_assembly):
        print(f"SKIP:\tFinal curated assembly already exists: {final_curated_assembly}")
    elif os.path.exists(curated_assembly):
        shutil.move(curated_assembly, final_curated_assembly)
        print(f"PASS:\tMoved curated assembly to {final_curated_assembly}")
    else:
        print(f"ERROR:\tCannot move {curated_assembly} to {final_curated_assembly}: curated assembly does not exist")
        return None
