...
def preprocess_refseq(sample_id, input_csv, output_dir, cpu_threads, ram_gb):
    ...
    log_print(f"Preprocessing Reference Sequence assembly for {sample_id.split('-')[0]}...")
                ...
                if target_fasta.exists():
                    try:
                        if src.resolve().samefile(target_fasta.resolve()):
                            log_print(f"PASS:\tReference sequence already in place: {target_fasta.resolve()}")
                        ...
                        if src.stat().st_size == target_fasta.stat().st_size:
                            log_print(f"PASS:\tReference sequence appears already normalized: {target_fasta.resolve()}")
                            return str(target_fasta.resolve())
                # Copy into expected location/name
                shutil.copy2(str(src), str(target_fasta))
                log_print(f"PASS:\tCopied REF_SEQ into expected location: {target_fasta.resolve()}")
                return str(target_fasta.resolve())
            except Exception as e:
                log_print(f"WARN:\tFailed to normalize provided REF_SEQ '{src}': {e}. Will try GCA if available.")
        else:
            log_print(f"ERROR:\tProvided REF_SEQ does not exist on disk: {src}. Will try GCA if available.")

    # Case B: Need (or fall back) to download via REF_SEQ_GCA
    if ref_seq_gca and ref_seq_gca.lower() != "nan":
        if target_fasta.exists():
            log_print(f"SKIP:\tREF_SEQ GCA already exists: {target_fasta.resolve()}")
            return str(target_fasta.resolve())
        ...
        log_print(f"PASS:\tSuccessfully placed Reference Sequence: {target_fasta.resolve()}")
        return str(target_fasta.resolve())

    # Fallback
    log_print(f"ERROR:\tNo reference sequence could be resolved for {sample_id}")
    return None
