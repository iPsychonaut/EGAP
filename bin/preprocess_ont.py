...
def preprocess_ont(sample_id, input_csv, output_dir, cpu_threads, ram_gb):
    ...
    """
    log_print(f"Preprocessing ONT reads for {sample_id}...")
    ...
    log_print(f"DEBUG - ont_raw_dir - {ont_raw_dir}")
    ...
    if pd.isna(ont_sra) and pd.isna(ont_raw_dir) and pd.isna(ont_raw_reads):
        log_print("SKIP:\tONT preprocessing; no reads provided")
        return None
    ...
    try:
        ...
        if isinstance(ont_raw_dir, str):
            log_print(f"NOTE:\tConcatenating ONT files from {ont_raw_dir}")
            ...
            if not ont_files:
                log_print(f"ERROR:\tNo ONT files found in {ont_raw_dir_abs}")
                sys.exit(1)
        ...
        if isinstance(ont_sra, str):
            if not ont_raw_reads or not os.path.exists(ont_raw_reads):
                log_print(f"Downloading SRA {ont_sra} from GenBank...")
                ...
                if not os.path.exists(dumped):
                    log_print("ERROR:\tExpected FASTQ not found after fasterq-dump.")
                    return None
                if os.path.getsize(dumped) == 0:
                    log_print(f"ERROR:\tDownloaded FASTQ is empty: {dumped}")
                    return None
                ont_raw_reads = dumped
                log_print(f"PASS:\tSRA converted to FASTQ: {ont_raw_reads}")
            else:
                log_print(f"SKIP:\tSRA already exists: {ont_raw_reads}")
        ...
        if not (isinstance(ont_raw_reads, str) and os.path.exists(ont_raw_reads)):
            log_print(f"ERROR:\tONT reads not found: {ont_raw_reads}")
            return None
        ...
        try:
            ...
            log_print(f"WARN:\tNanoPlot failed on raw ONT reads ({e}); continuing without NanoStats.")
        ...
        if not os.path.exists(filtered_ont):
            ...
            if (not os.path.exists(filtered_ont)) or os.path.getsize(filtered_ont) == 0:
                log_print(f"ERROR:\tFiltlong did not produce reads at {filtered_ont}")
                return None
        else:
            log_print(f"SKIP\tFiltlong filtered reads exist: {filtered_ont}.")
        ...
        try:
            ...
            log_print(f"WARN:\tNanoPlot failed on filtered ONT reads ({e}); continuing.")
    
        # Ratatosk (only if Illumina reads available)
        corrected_out = os.path.join(ont_dir_abs, "ratatosk_corrected")
        if (not use_illumina):
            log_print("SKIP:\tRatatosk correction; no usable Illumina reads provided")
            ...
            if not os.path.exists(final_corrected_ont):
                ...
                if os.path.exists(corrected_out + ".fastq"):
                    ...
                    log_print(f"WARN:\tRatatosk did not produce {corrected_out}.fastq; using filtered reads.")
                    final_corrected_ont = filtered_ont
        ...
        try:
            ...
            log_print(f"WARN:\tNanoPlot failed on corrected ONT reads ({e}); continuing.")
    ...
    log_print(f"PASS:\tPreprocessed Raw ONT Reads for {sample_id}: {highest_mean_qual_long_reads}.")
